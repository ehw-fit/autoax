...
df_random = df_random.groupby("configuration").agg(aggf)
...
def do_learn(df_random: pd.DataFrame, df_lib: pd.DataFrame, mcm: int, x_params=["bddavg", "bddmax"], y_param="total_psnr_yuv"):
    ...
    for reg, regname in [
        ...
        joblib.dump(
            reg, f'reg/regressor.{regname}.mcm{mcm}_{y_param}.joblib')
        ...
        plt.savefig(f"plt/regressor.{regname}.{mcm}_{y_param}.png")
        plt.close()
    return corr_out
...
if __name__ == "__main__":
    ...
    df_corr.to_csv("reg/corr_random.csv")
    df_corr.to_pickle("reg/corr_random.pkl.gz")
