...
aggf = {x: issame if x == "mcm" or df_random[x].dtype == np.dtype(
    "O") else "mean" for x in df_random.columns}
df_random = df_random.query(
    "benchmark=='bridge-close'").groupby("configuration").agg(aggf)


# %%%

def do_learn(df_random: pd.DataFrame, df_lib: pd.DataFrame, mcm: int, x_params=["bddavg", "bddmax"], abc_params=[], y_param="total_psnr_yuv"):
    # prepare X set
    dfr = df_random.query("mcm == @mcm").dropna(axis="columns", how="all")

    config_colums = [x for x in dfr.columns if x.startswith("dct_") or x.startswith(f"c{mcm}_")]
    print(config_colums)
    ...
    if abc_params:
        abc = abcsynth.ABCsynth()
        for i, r in tqdm(list(dfr.iterrows()), "Synth"):
            conf = { k:r[k] for k in config_colums }
            conf["mcm"] = mcm
            #print("conf", conf)
            ret = abc.runSynth( conf )
            #print(ret)
            for k in abc_params:
                dfr.loc[i, k] = ret[k]

        x_columns += abc_params
    ...
    for reg, regname in [
        ...
        joblib.dump(
            reg, f'reg/bridge-abcregressor.{regname}.mcm{mcm}_{y_param}.joblib')
        ...
        plt.savefig(f"plt/bridge-abcregressor.{regname}.{mcm}_{y_param}.png")
        plt.close()
    return corr_out
...
if __name__ == "__main__":
    ...
    for mcm in [1, 2, 3, 4]:
        ...
        df_corr += do_learn(df_random=df_random, df_lib=df_lib, mcm=mcm, x_params=["abc_power"],
                            abc_params=["abc_lat", "abc_nd", "abc_edge", "abc_aig", "abc_lev", "abc_power"], y_param="power")
    df_corr = pd.DataFrame(df_corr)
    df_corr.to_csv("reg/bridge-abc-corr_random.csv")
    df_corr.to_pickle("reg/bridge-abc-corr_random.pkl.gz")
