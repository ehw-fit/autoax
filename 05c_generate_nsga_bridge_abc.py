...
def do_hc(mcm = 1, rnd = 10000, p_size=200, q_size = 100, mutation_rate=0.1):
    ...
    for rid in tqdm(range(rnd), f"NSGA search mcm={mcm}"):
        ...
        if rid % (rnd / 10) == 0:
            ...
            df.to_pickle(f"data/bridge-abc-nsga_mcm{mcm}_{rnd}x{p_size}_gen_{rid}.pkl.gz")

            plt.scatter(df["est_hw"], df["est_qual"])
            plt.xlim(0, None)
            plt.ylim(0, None)
            plt.savefig(f"data/bridge-abc-nsga_mcm{mcm}_{rnd}x{p_size}_gen_{rid}.png")
            plt.close()
    ...
    for x in parent:
        while True:
            cid = f"nsga_abc-bridge_mcm{mcm}_" + uuid1().hex[:8].upper()
            if not cid in outc:
                break
    ...
    pd.DataFrame(sl).to_pickle(f"data/bridge-abc-nsga_mcm{mcm}_{rnd}x{p_size}.pkl.gz")
    #xxxx
